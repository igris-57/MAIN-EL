...
def data_downloader_multi(start_date, end_date, site, collector_ripe='rrc04', collector_routeviews='route-views2'):
    ...
    if site == 'RIPE':
        for date_i in date_list:
            ...
            if os.path.exists("./src/data_split/DUMP_%s_out.txt" % date_i):
                print("\\n RIPE => >>>>>>>>>> Skipping download, already exists: DUMP_%s_out.txt" % date_i)
                continue
            ...
            subprocess_cmd("cd src/; \
                            cd data_ripe/%s/ ; \
                            wget -4 -e robots=off -r -np -nd -A '%s' %s ;" % (date_i, data_file, data_link))
    ...
    # collector_routeviews is not finished (only use 'route-views2' now)
        ...
        for date_i in date_list:
            # SKIP if already done
            if os.path.exists("./src/data_split/DUMP_%s_out.txt" % date_i):
                print("\\n RouteViews => >>>>>>>>>> Skipping download, already exists: DUMP_%s_out.txt" % date_i)
                continue
