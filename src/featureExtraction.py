...
def feature_extractor_multi(start_date, end_date, site):
    ...
    if site == 'RIPE':
        ...
        for date_i in date_list:
            ...
            output_file = "%s_out.txt" % file_name
            if os.path.exists("./src/data_split/" + output_file):
                print("\n RIPE => >>>>>>>>>> Skipping extraction, already exists: %s" % output_file)
        ...
        for date_i in date_list:
            ...
            output_file = "%s_out.txt" % file_name
            if os.path.exists("./src/data_split/" + output_file):
                print("\n RouteViews => >>>>>>>>>> Skipping extraction, already exists: %s" % output_file)
                output_file_list.append(output_file)
                continue
