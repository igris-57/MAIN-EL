...
def analyze_offline():
    ...
    if model_params == set(request.form.keys()):
        ...
        def run_offline_task():
            ...
            app_offline_classification(header_offLine, input_exp_key)
        ...
        thread = threading.Thread(target=run_offline_task)
        thread.start()

        # Instantly return to prevent browser timeout
        context_offLine = {"result_prediction": input_exp_key,
                           "site_selected": "Processing started in background! Please wait 10-15 minutes, then refresh the page and click Download.",
